...
export const getActive = async (req, res) => {
  try {
    ...
    const enriched = challenges.map(c => {
      ...
      const participation = obj.participants.find(p => p.userId.toString() === req.userId);
...
export const create = async (req, res) => {
  try {
    const { name, description, type, target, unit, startDate, endDate, image } = req.body;
    if (!name || !target || !startDate || !endDate) {
      return res.status(400).json({ error: 'Name, target, startDate, and endDate are required' });
    }

    const challenge = await Challenge.create({
      name, description, type: type || 'WORKOUT_COUNT', target, unit,
      startDate, endDate, image, createdBy: req.userId, isPublic: true,
...
export const join = async (req, res) => {
  try {
    ...
    if (new Date() > challenge.endDate) return res.status(400).json({ error: 'Challenge has ended' });

    challenge.participants.push({ userId: req.userId, progress: 0 });
    await challenge.save();

    res.status(200).json({ message: 'Joined', participantCount: challenge.participants.length });
  } catch (error) {
    res.status(500).json({ error: 'Internal server error' });
  }
};
  ...
  try {
    ...
    if (progress >= challenge.target && !participant.completedAt) {
      ...
      await Notification.create({
        userId: req.userId, type: 'ACHIEVEMENT',
    ...
    res.status(200).json({ progress: participant.progress, completed: !!participant.completedAt });
  } catch (error) {
    res.status(500).json({ error: 'Internal server error' });
  }
};
  ...
  try {
    ...
    const leaderboard = challenge.participants
      .map(p => ({
        ...
        percentage: Math.min(100, Math.round((p.progress / challenge.target) * 100)),
      }))
      .sort((a, b) => b.progress - a.progress);

    res.status(200).json({ leaderboard, target: challenge.target, unit: challenge.unit });
  } catch (error) {
    res.status(500).json({ error: 'Internal server error' });
  }
};
